...
$fileType = pathinfo($targetFilePath,PATHINFO_EXTENSION);
    ...
    if(in_array($fileType, $allowTypes)){
        ...
        if(move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)){
            ...
            $query="UPDATE images SET file_name = '$targetDir$fileName', uploaded_on = NOW() WHERE p_id = '$pid'";
            $insert =mysqli_query($con, $query);
            if($insert){
                $statusMsg = "The product has been updated successfully.";
            }else{
...
// Display status message
             ...
             echo 'setTimeout(function () { swal.fire({title:"Information
             ",text: "'.$statusMsg.'",type: "info",showConfirmButton: false });';
             echo '}, 1000);</script>';
...
function add_product($amount,$pname,$price,$description) {     
      $con=mysqli_connect("localhost","root","","emarketlk");
      $pid=$_POST['id'];
      $query = "UPDATE product SET amount = '$amount',item = '$pname', price='$price', description ='$description' WHERE p_id='$pid'";
...
<head>
    ...
    <title>Update Products| E-Market</title>
